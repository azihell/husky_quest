...
class Camera:
  ...
  def color_detector(self, cv_image):
    ...
    if len(cnt_yellow) > 0:
      ...
      cv2.circle(img_rgb, (int(centers[0][0]), int(centers[0][1])), int(radius[0]), (0, 255, 255), 2)
  ...
  def run(self):
    self.sub_image = rospy.Subscriber("/husky_cam/image_raw", Image, self.image_callback)
